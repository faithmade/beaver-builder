...
final class FLBuilderModel {
	...
	static public function copy_row($node_id = null)
	{
		...
		// Only add child nodes if this isn't a global row.
		if ( ! self::is_node_global( $row ) ) {
	 ...
	 * @param array $template_data Template data for making this module a copy of a global node template. 
	 * @return object The new module object.
	 */
	static public function add_module($type = null, $settings = array(), $parent_id = null, $position = false, $template_data = null )
	{
		$post_data			= self::get_post_data();
		$data				= self::get_layout_data();
		...
		if ( self::is_node_global( $parent ) ) {
			...
			$data[$module_node_id]->template_id 	  	= $template_data['template_id'];
			$data[$module_node_id]->template_node_id  	= $template_data['template_node_id'];
			$data[$module_node_id]->template_root_node  = true;
		}
	...
	static public function copy_module($node_id = null)
	{
		...
		return self::add_module( $module->settings->type, $module->settings, $module->parent, $module->position + 1, $template_data );
	}
